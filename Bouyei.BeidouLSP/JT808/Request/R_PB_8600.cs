   ...
   public class R_PB_8600
    {
        ...
        {
            List<byte> list = new List<byte>((info.circleItemsPB.Count * 33) + 1);

            list.Add(info.settingProperty);

            list.Add((byte)info.circleItemsPB.Count);

            for (int i = 0; i < list[1]; ++i)
            {
                list.AddRange(CircleAreaItem(info.circleItemsPB[i]));
            }
            return list.ToArray();
        }
