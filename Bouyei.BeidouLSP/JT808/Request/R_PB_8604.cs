   ...
   public class R_PB_8604
    {
        ...
        {
            List<byte> list = new List<byte>((info.polygonItemsPB.Count << 3) + 21);
            ...
            //区域顶点项
            list.Add((byte)info.polygonItemsPB.Count);
            for (int i = 0; i < info.polygonItemsPB.Count; ++i)
            {
                list.AddRange(info.polygonItemsPB[i].Value.ToBytes());
                list.AddRange(info.polygonItemsPB[i].UInt32Value.ToBytes());
            }
            return list.ToArray();
        }
