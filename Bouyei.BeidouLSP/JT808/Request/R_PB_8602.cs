   ...
   public class R_PB_8602
    {
        ...
        {
            List<byte> buffer = new List<byte>((info.rectangleItemsPB.Count * 33) + 1);

            buffer.Add(info.settingProperty);

            buffer.Add((byte)info.rectangleItemsPB.Count);

            for (int i = 0; i < buffer[1]; ++i)
            {
                buffer.AddRange(RectangleAreaItem(info.rectangleItemsPB[i]));
            }
